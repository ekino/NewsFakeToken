...
const customStyleContent = (node) => {
  const stylesheet =
    node.getAttribute("stylesheet") || `${__dirname}/slides.css`;
...
const sectionInlineStyle = (node) => {
  ...
  if (image) {
    ...
    return `style="background-image: url(${node.getImageUri(
...
const sectionTitle = (node) => {
  const titleSeparator =
    node.getDocument().getAttribute("title-separator") || ":";
...
module.exports = {
  paragraph: (node) =>
    `<p class="${node.getRoles().join(" ")}">${node.getContent()}</p>`,
  section: (node) => `<section class="${sectionRoles(node).join(" ")} ${
    node.getTitle() === "!" ? "no-title" : ""
  }" data-slide-number="${node.index + 1}" data-slide-count="${
    node.parent.blocks.length
...
${node.getContent()}
  ...
  open: (node) =>
    `<div${elementId(node)} class="${node
      .getRoles()
      .join(" ")}">${node.getContent()}</div>`,
  image: (node) => {
    const roles = node.getRoles();
    ...
    return `<figure class="image ${node
      .getRoles()
      .join(" ")}"><img src="${node.getImageUri(
      node.getAttribute("target")
    )}" height="${height}" width="${width}"/></figure>`;
  },
};
